...
export class CharRifleman extends CharBase{
    ...
    public useAbility(i:number){
        ...
        ProjSystem.GetIns().addProjectileAngled(
            ...
            AddSpecialEffect("Abilities\\Weapons\\FarseerMissile\\FarseerMissile.mdl",0,0),
            () => {
    ...
    public constructor(pid:number) {
        ...
        PlayerData.GetIns().height[pid] = PlayerData.GetIns().height[pid];
        PlayerData.GetIns().SetPlayerModel(pid, "units\\human\\Rifleman\\Rifleman.mdl");
        BlzSetSpecialEffectColorByPlayer(PlayerData.GetIns().model[pid],Player(pid));
        BlzSetSpecialEffectScale(PlayerData.GetIns().model[pid], 2);
        this.abiButton = [];
        this.abiButton.push(new AbilityButton(pid,5,"ReplaceableTextures\\CommandButtons\\BTNChainLightning.blp", "|cffffcc00H",0));
        this.abiButton.push(new AbilityButton(pid,0.5,"ReplaceableTextures\\CommandButtons\\BTNGatherGold.blp", "|cffffcc00B",0.06));
        this.abiButton.push(new AbilityButton(pid,0.5,"ReplaceableTextures\\CommandButtons\\BTNBootsOfSpeed.blp", "|cffffcc00N",-0.06));
